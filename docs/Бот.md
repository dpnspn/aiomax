...
> [!TIP]
> Для более легкого изменения команд в приложении можно использовать `Bot.sync_commands()`.
...
Изменяет список команд в интерфейсе приложения на команды, созданные через декоратор `on_command`, с указанными параметрами `description`.
