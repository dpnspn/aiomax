...
### `Bot(access_token: str, command_prefixes: str | List[str] = '/', mention_prefix: bool = True, case_sensitive: bool = True, default_format: Literal['markdown', 'html'] | None = None, max_messages_cached: int = 10000, autosync_commands: bool = True)`
...
- `autosync_commands: bool` - Если `True`, при запуске бота автоматически вызовется `Bot.sync_commands`.
...
- `commands: List[BotCommand] | None` - новый список команд бота, отображаемый в приложении при вводе `/` в поле ввода сообщения

- `photo: ImageRequestPayload | None` - новая аватарка бота в виде объекта класса `ImageRequestPayload`

### `Bot.sync_commands() -> List[BotCommand]`

Отправляет на сервер все добавленные команды через декоратор `on_command`, у которых указан `description`, и возвращает их.

Синхронизированные команды отображаются во встроенном интерфейсе приложения при вводе `/` в поле ввода сообщения.

Если `Bot.autosync_commands` является `True`, вызовется автоматически при запуске бота.
