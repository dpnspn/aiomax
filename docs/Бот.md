...
### `Bot.start_polling(session: "aiohttp.ClientSession | None" = None, use_certificate: bool = False)`
...
- `use_certificate: bool` - если `True`, то [сертификат Минцифры](https://www.gosuslugi.ru/crt) будет использован автоматически при подключении к Максу.

### `Bot.run(use_certificate: bool = False)`
