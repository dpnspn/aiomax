...
> [!NOTE]
> Фильтры могут быть как и синхронные так и асинхронные
...
> [NOTE]
> Для использования встроенных фильтров и иных дочерних классу `BaseFilter`, Вам может быть удобнее использовать [Магию](#магическое-использование-нескольких-фильтров)
...
## Магические функции фильтров

Магические функции - функции класса `BaseFilter`, позволяющие проводить логические операции с фильтрами, дочерними ему
...
Поддерживаются бинарные операции `&` для логического И (and) и `|` для логического ИЛИ (or)
...
Проверка на то, чтобы любой фильтр возвращал `True`:
...
В `BaseFilter` (родительный объект всех встроенных фильтров) добавлена функция для отрицания фильтров.
