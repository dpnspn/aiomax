...
- `original_obj: any` - объект, переданный хендлеру, в котором возникло исключение (например, сообщение, контекст или другая полезная нагрузка)
...
Является функцией `send` у `original_obj`. Если у `original_obj` таковой нет, функция ничего не делает.
...
Является функцией `reply` у `original_obj`. Если у `original_obj` таковой нет, но есть функция `send`, будет вызвана она. В другом случае функция ничего не делает.
