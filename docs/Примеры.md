...
> Если у вас не установлен сертификат, в параметрах к `aiomax.Bot()` можно указать `use_certificate=True` - тогда aiomax будет использовать встроенный сертификат Минцифры.
