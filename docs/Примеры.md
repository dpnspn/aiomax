...
# Команда, поднимающая ошибку с шансом 50%
@bot.on_command('random')
async def command(ctx: aiomax.CommandContext):
    if random.random() < 0.5:
        raise Exception('Точно в цель!')
        
    await ctx.reply('Нет ошибки')

# Обработка ошибки
@bot.on_exception()
async def handling_error(e: Exception, event = None, *args, **kwargs):
    # пытаемся отправить пользователю сообщение об ошибке
    try:
        await event.send(f'Ошибка - {e}')
    # не получилось отправить
    # чаще всего происходит потому, что мы получиши event,
    # в котором нет метода send
    except:
        pass
