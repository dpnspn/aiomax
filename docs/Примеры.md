...
command_router.add_message_filter(aiomax.filters.StartsWithFilter('$'))
...
async def start(pd: aiomax.BotStartPayload, cursor: fsm.FSMCursor):
    ...
    cursor.change_state('name') # Изменения состояние для обработки сообщения с именем
...
async def write_name(message: aiomax.Message, cursor: fsm.FSMCursor):
    ...
    cursor.update_data(name=message.content) # Добавление имени в данные пользователя, чтобы получать его в следующей функции
...
async def write_surname(message: aiomax.Message, cursor: fsm.FSMCursor):
    ...
    cursor.clear() # очистка данных и состояния пользователя
...
# Команда генерации случайных чисел
@bot.on_command('random')
async def gen(ctx: aiomax.CommandContext):
    min_num = int(ctx.args[0])
    max_num = int(ctx.args[1])
    number = random.randint(min_num, max_num)
...
# Обработка ошибки - сработает если введется не число или первое число не будет меньше второго
@bot.on_exception()
async def handling_error(ctx: aiomax.ExceptionContext):
    await ctx.reply(f'❌ **Некорректные аргументы!**\n\n{ctx.exception}')

bot.run()
```

## Команда с проверкой по чату
...
class ChatFilter(aiomax.filters.BaseFilter):
    def __init__(self, chat_id: int):
        self.chat_id = chat_id
    
    def __call__(self, ctx: aiomax.CommandContext):
        return ctx.recipient.chat_id == chat_id

chat_id = 2409 # Айди чата для проверки

@bot.on_command("test", ChatFilter(chat_id))
async def test_command(ctx: aiomax.CommandContext):
    pass
